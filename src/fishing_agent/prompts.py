...
- 无需额外调用 get_weather
- **必须主动总结天气数据**（温度、天气状况、风力等）告知用户
- 用自然语言整合钓鱼推荐和天气分析，确保用户了解当天天气情况
...
- 无需额外调用 get_weather
- **必须主动总结天气数据**（温度、天气状况、风力、气压等）
- 用自然语言回答"天气是否适合钓鱼"，确保用户充分了解天气情况
...
- 每次查询只调用一个工具
- 基于真实数据，不编造信息
- **使用 query_fishing_recommendation 时，必须在回复中总结天气数据**（温度、天气、风力等）
