...
def _parse_weather_summaries(weather_summaries: dict) -> dict:
    """解析天气汇总数据，提取关键信息
...
def _format_overall_weather(parsed_weather: dict) -> str:
    """格式化总体天气情况
...
def _format_time_slots(best_slots: list, weather_summaries: dict) -> str:
    """格式化时间段推荐
...
def _execute_fishing_analysis(location: str, date: str) -> str:
    """执行钓鱼分析异步操作
...
def _parse_fishing_result(result: str) -> dict:
    """解析钓鱼分析结果
...
def _build_fishing_response(location: str, data: dict) -> str:
    """构建钓鱼推荐的完整响应
    ...
    if weather_summaries:
        parsed_weather = _parse_weather_summaries(weather_summaries)
        overall_weather = _format_overall_weather(parsed_weather)
        if overall_weather:
            response += "🌤️ **总体天气情况**:\n"
    ...
    time_slots = _format_time_slots(best_slots, weather_summaries)
    response += time_slots
...
def query_fishing_recommendation(location: str, date: str = None) -> str:
    ...
    try:
        # 执行钓鱼分析
        result = _execute_fishing_analysis(location, date)

        # 解析结果
        data = _parse_fishing_result(result)

        # 构建响应
        return _build_fishing_response(location, data)
