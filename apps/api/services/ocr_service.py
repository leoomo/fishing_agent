...
class OCRService:
    ...
    使用硅基流动 DeepSeek-OCR 模型识别装备规格表等图片中的表格内容，
    返回 Markdown 格式的表格。
    """
    ...
    def __init__(self):
        ...
        logger.info(f"OCRService initialized: timeout={self.timeout}s, max_size={self.max_size/1024/1024:.1f}MB")

    def _validate_api_key(self) -> None:
        """验证 API 密钥是否已配置"""
        if not self.api_key:
            raise OCRError("未配置硅基流动 API 密钥，请在 .env 中设置 SILICONFLOW_API_KEY", "OCR_API_KEY_MISSING")
    ...
    def _call_api(self, payload: dict, verbose: bool = False) -> dict:
        ...
        for attempt in range(self.max_retries):
            try:
                if verbose:
                    logger.info(f"API 请求 (尝试 {attempt + 1}/{self.max_retries})...")
                ...
                last_error = OCRError(f"API 请求超时 ({self.timeout}秒)", "OCR_TIMEOUT")
                logger.warning(f"请求超时，第 {attempt + 1} 次重试...")
                time.sleep(1)

            except requests.RequestException as e:
                last_error = OCRError(f"API 请求失败: {str(e)}", "OCR_API_ERROR")
    ...
    def _parse_response(self, response: dict) -> str:
        ...
        if "choices" not in response or not response["choices"]:
            raise OCRError("API 响应格式错误：无 choices", "OCR_PARSE_ERROR")

        content = response["choices"][0].get("message", {}).get("content", "")

        if not content:
            raise OCRError("API 响应内容为空", "OCR_PARSE_ERROR")
    ...
    def detect_text_in_region(self, image_path: str, region: Tuple[float, float, float, float]) -> dict:
        ...
        try:
            ...
            if not os.path.exists(image_path):
                raise OCRError(f"图片文件不存在: {image_path}", "OCR_FILE_NOT_FOUND")

            # 验证区域参数
            x_min, y_min, x_max, y_max = region
            if not (0 <= x_min <= x_max <= 1 and 0 <= y_min <= y_max <= 1):
                raise OCRError("区域坐标无效，必须是0-1之间的相对坐标", "OCR_INVALID_REGION")
            ...
            with Image.open(image_path) as img:
                ...
                logger.debug(f"Text detection stats for {Path(image_path).name}: "
    ...
    def _merge_images(self, image_paths: List[str]) -> Tuple[str, int]:
        ...
        if not success:
            raise OCRError("图片合并失败", "OCR_MERGE_ERROR")
    ...
    def recognize_table_from_paths(
        ...
        try:
            ...
            if not image_paths:
                raise OCRError("未提供图片", "OCR_NO_INPUT")

            # 2. 验证所有图片格式
            for path in image_paths:
                if not os.path.exists(path):
                    raise OCRError(f"图片文件不存在: {path}", "OCR_FILE_NOT_FOUND")
            ...
            processing_time_ms = int((time.time() - start_time) * 1000)
            logger.error(f"OCR 错误: {e.message} ({e.error_code})")
            return {
                "success": False,
                "markdown": None,
                "metadata": {
