...
interface UseUserSearchReturn {
  ...
  resetFilters: () => void
  // 用于 API 请求的参数
  searchParams: UserSearchFilters & { page: number; page_size: number }
}

// URL 参数键映射
const URL_PARAM_KEYS: (keyof UserSearchFilters)[] = [
  'keyword',
  'user_level',
  'fishing_experience_min',
  'fishing_experience_max',
  'preferred_fishing_method',
  'created_after',
  'created_before',
]
...
export function useUserSearch(
  ...
  // 数值类型参数
  const INT_PARAMS: (keyof UserSearchFilters)[] = [
    'fishing_experience_min',
    'fishing_experience_max',
  ]

  // 从 URL 解析初始筛选条件
  const parseFiltersFromURL = useCallback((): UserSearchFilters => {
  ...
  // 状态
  const [filters, setFiltersState] = useState<UserSearchFilters>(parseFiltersFromURL)
  ...
  const syncToURL = useCallback(
    (newFilters: UserSearchFilters, newPage: number, newPageSize: number) => {
