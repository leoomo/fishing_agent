...
# 装备数量趋势（按月）
curl "http://localhost:8000/api/v1/admin/analytics/equipment/trends?months=12" \
  -H "Authorization: Bearer <your_jwt_token>"

# 价格分布统计
curl "http://localhost:8000/api/v1/admin/analytics/equipment/price-distribution" \
  -H "Authorization: Bearer <your_jwt_token>"

# 品牌统计排行
curl "http://localhost:8000/api/v1/admin/analytics/equipment/brand-stats?top_n=10" \
  -H "Authorization: Bearer <your_jwt_token>"

# 用户活跃度统计
curl "http://localhost:8000/api/v1/admin/analytics/users/activity?days=30" \
  -H "Authorization: Bearer <your_jwt_token>"
...
# 查询报表列表
curl "http://localhost:8000/api/v1/admin/analytics/reports/list?page=1&page_size=10" \
  -H "Authorization: Bearer <your_jwt_token>"

# ========== 配置管理API ⭐ v5.0.0新增 ==========

# 查询配置列表
curl "http://localhost:8000/api/v1/admin/config/configs?type=api&page=1&page_size=20" \
  -H "Authorization: Bearer <your_jwt_token>"

# 获取特定配置
curl "http://localhost:8000/api/v1/admin/config/configs/CAIYUN_API_KEY" \
  -H "Authorization: Bearer <your_jwt_token>"
...
# 更新配置
curl -X PUT "http://localhost:8000/api/v1/admin/config/configs/CAIYUN_API_KEY" \
  -H "Authorization: Bearer <your_jwt_token>" \
  -H "Content-Type: application/json" \
  -d '{
...
# 删除配置
curl -X DELETE "http://localhost:8000/api/v1/admin/config/configs/OLD_CONFIG" \
  -H "Authorization: Bearer <your_jwt_token>"
...
> ✅ **v5.0.0更新**: Phase 5 数据分析和配置管理模块完成！
> 📈 **v5.0.0新增**: 数据分析报表 + 装备统计趋势分析！
> ⚙️ **v5.0.0新增**: 系统配置管理 + API密钥测试功能！
> 🖥️ **v5.0.0新增**: React管理前端 + 17条装备数据成功展示！
...
### 当前分支状态
> ✅ **feature/equipment-ui分支已完成** - React管理前端、JWT认证系统、装备管理UI优化和Phase 5数据分析配置模块已实现
> - ✅ Phase 5完成：数据分析报表 + 系统配置管理 + React管理前端
> - ✅ React前端：完整的管理界面，17条装备数据展示成功
> - ✅ 前后端集成：API服务器(8000端口) + React前端(5174端口)正常运行
