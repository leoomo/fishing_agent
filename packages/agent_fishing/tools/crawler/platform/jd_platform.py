...
class JDPlatform(BasePlatform):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.platform_name = "jd"

        # 动态导入RPA模块（避免循环依赖）
        try:
            from packages.agent_fishing.tools.crawler.rpa.jd_rpa import JDRPA
            self.rpa = JDRPA()
        except ImportError as e:
            logger.warning(f"无法导入JDRPA模块: {e}")
            self.rpa = None
    ...
    async def crawl(self,
        ...
        if not self.rpa:
            # 如果京东RPA模块不存在，返回模拟数据
            logger.warning("JDRPA模块未正确初始化，返回模拟数据")
            return await self._simulate_crawl(task_config, on_progress)
    ...
    async def _simulate_crawl(self,
                            task_config: TaskConfig,
        ...
        if task_config.task_type == TaskType.KEYWORD_SEARCH:
            for keyword in task_config.keywords or []:
                for i in range(3):  # 每个关键词返回3个模拟商品
                    mock_items.append({
                        "name": f"京东-{keyword}-商品{i+1}",
                        "category": "渔具",
                        "brand": "京东品牌",
                        "model": f"JD-{keyword}-Model{i+1}",
                        "price_min": 100.0 + i * 50,
                        "price_max": 200.0 + i * 50,
                        "description": f"京东平台{keyword}相关商品",
                        "source_url": f"https://item.jd.com/123456{i+1}.html",
                        "platform": "jd"
                    })
                on_progress(f"模拟完成关键词 '{keyword}'", 50)

        on_progress("京东模拟爬取完成", 100)
    ...
    async def _crawl_by_keywords(self,
                               ...
                               on_progress: Optional[Callable[[str, int], None]] = None) -> Dict[str, Any]:
        """关键词搜索爬取"""
        logger.info(f"开始京东关键词搜索: {task_config.keywords}")
    ...
    async def _crawl_shop(self,
                         ...
                         on_progress: Optional[Callable[[str, int], None]] = None) -> Dict[str, Any]:
        """店铺爬取"""
        logger.info(f"开始京东店铺爬取: {task_config.shop_url}")
    ...
    async def _crawl_product_details(self,
                                   ...
                                   on_progress: Optional[Callable[[str, int], None]] = None) -> Dict[str, Any]:
        """商品详情爬取"""
        logger.info(f"开始京东商品详情爬取，共 {len(task_config.product_urls or [])} 个商品")
