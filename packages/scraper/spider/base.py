...
class CrawlItem:
    ...
    原名 EquipmentData，重命名为更通用的 CrawlItem
    """
    ...
    target_fish: Optional[str] = None  # 目标鱼种（钓鱼业务特定）
...
class BaseSpider(ABC):
    ...
    @abstractmethod
    def crawl(self, **kwargs) -> List[CrawlItem]:
        """
        执行爬取操作
    ...
    @abstractmethod
    def get_product_detail(self, product_url: str) -> Optional[CrawlItem]:
        """
        获取商品详情
    ...
    def fetch_with_retry(
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if response and response.status_code in [404, 403, 401]:
                    # 不重试的错误
                    break
